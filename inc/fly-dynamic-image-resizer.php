...
Source: Fly Dynamic Image Resizer by Junaid Bhura
*/
if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

class UPT_Fly_Images {
...
if ( ! function_exists( 'upt_fly_add_image_size' ) ) {
	/**
	 ...
	 * @param boolean  $crop
	 */
	function upt_fly_add_image_size( $size_name = '', $width = 0, $height = 0, $crop = false ) {
		global $upt_fly_images;
		return $upt_fly_images->add_image_size( $size_name, $width, $height, $crop );
	}
}

if ( ! function_exists( 'upt_fly_get_attachment_image_src' ) ) {
	/**
	 * Gets a dynamically generated image URL from the Fly_Images class.
	 *
	 * @param  integer  $attachment_id
	 ...
	 * @return string
	 */
	function upt_fly_get_attachment_image_src( $attachment_id = 0, $size = '', $crop = null ) {
		global $upt_fly_images;
		return $upt_fly_images->get_attachment_image_src( $attachment_id, $size, $crop );
	}
}

if ( ! function_exists( 'upt_fly_get_attachment_image' ) ) {
	/**
	 * Gets a dynamically generated image HTML from the Fly_Images class.
	 *
	 * @param  integer  $attachment_id
	 ...
	 * @return string
	 */
	function upt_fly_get_attachment_image( $attachment_id = 0, $size = '', $crop = null, $attr = array() ) {
		global $upt_fly_images;
		return $upt_fly_images->get_attachment_image( $attachment_id, $size, $crop, $attr );
	}
}

// Initialize Plugin!
global $upt_fly_images;
$upt_fly_images = new UPT_Fly_Images();
